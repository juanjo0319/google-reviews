...
import { saveBrandVoice } from "@/app/actions/settings";
...
export function BrandVoiceForm({
  ...
  orgId: string;
  initialData: BrandVoiceData | null;
}) {
  const [tone, setTone] = useState(initialData?.tone ?? "professional and friendly");
  const [formality, setFormality] = useState(initialData?.formality ?? 7);
  const [humor, setHumor] = useState(initialData?.humor_level ?? 3);
  const [useEmoji, setUseEmoji] = useState(initialData?.use_emoji ?? false);
  const [signature, setSignature] = useState(initialData?.signature_name ?? "");
  const [preferred, setPreferred] = useState<string[]>(
    initialData?.preferred_phrases ?? ["Thank you for your feedback", "We appreciate your support"]
  );
  const [avoid, setAvoid] = useState<string[]>(
    initialData?.avoid_phrases ?? ["sorry for the inconvenience"]
  );
  const [responseLength, setResponseLength] = useState(initialData?.response_length ?? "2-4 sentences");
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const result = await saveBrandVoice(orgId, {
        tone,
        formality,
        humor_level: humor,
        use_emoji: useEmoji,
        ...
        custom_examples: initialData?.custom_examples ?? null,
        values,
      });
      if (result.success) {
        setSaved(true);
